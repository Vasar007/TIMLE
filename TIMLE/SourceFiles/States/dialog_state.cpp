...
#include "States/dialog_state.hpp"
...
, _playerInfo(*context.mPlayerInfo)
{
    ...
    switch(_playerInfo.mDialogNumber)
    {
        ...
        case 8:
            ...
            else if (_playerInfo.mChosenSolution.at(PlayerInfo::Solution::LootKnightAndDwarven) == 2)
            {
            ...
            {
                addText(L"Трудно сказать наверняка, что он мог тут делать. Лишь в одном вы уверены – монстр здесь стоит не  просто так. ", L"Повествование");
            }
            break;
        case 9:
    ...
    {
        if (_playerInfo.mDialogNumber == 7 &&
            _playerInfo.mChosenSolution.at(PlayerInfo::Solution::LootKnightAndDwarven) == 0)
        {
            _playerInfo.mChoosingNumber = 1;
            requestStackPush(States::ID::Choosing);
            _choosing = true;
        }
        else if (_playerInfo.mDialogNumber == 7 &&
                 _playerInfo.mChosenSolution.at(PlayerInfo::Solution::LootKnightAndDwarven) != 0 &&
                 _textNumber == 0)
        {
            if (_playerInfo.mChosenSolution.at(PlayerInfo::Solution::LootKnightAndDwarven) == 1)
        ...
        else if (_playerInfo.mDialogNumber == 8 &&
                 _playerInfo.mChosenSolution.at(PlayerInfo::Solution::InteractWithGolem) == 0 &&
                 _textNumber == 1)
        {
            ...
            _choosing = true;
                 ...
                 _textNumber == 1)
        {
            if (_playerInfo.mChosenSolution.at(PlayerInfo::Solution::InteractWithGolem) == 1)
            {
            ...
            else if (_playerInfo.mChosenSolution.at(PlayerInfo::Solution::InteractWithGolem) == 2)
            {
            ...
            else if (_playerInfo.mChosenSolution.at(PlayerInfo::Solution::InteractWithGolem) == 3)
            {
...
{
    auto& text = _dialogText.at(number);
    ...
    _talking.setString(_dialogTalking.at(number));
}
...
bool DialogState::update(const sf::Time) noexcept
{
    return false;
}
