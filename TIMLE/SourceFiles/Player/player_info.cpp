...
const std::map<PlayerInfo::Quest, bool> PlayerInfo::_defaultQuestValues{
    { Quest::TalkWithOswald,        false },
...
const std::map<PlayerInfo::Solution, std::size_t> PlayerInfo::_defaultSolutionValues{
...
{
    if (const auto it = std::find(std::begin(mShowedDialogs), std::end(mShowedDialogs), number);
        it == std::end(mShowedDialogs))
    {
...
void PlayerInfo::assignKey(const Action action, const sf::Keyboard::Key key)
{
    ...
    for (auto it = std::begin(_keyBinding); it != std::end(_keyBinding);)
    {
        if (it->second == action)
        {
...
void PlayerInfo::setGameStatus(const GameStatus status) noexcept
{
    _currentGameStatus = status;
}

PlayerInfo::GameStatus PlayerInfo::getGameStatus() const noexcept
{
    return _currentGameStatus;
}

void PlayerInfo::setLevelNumber(const std::size_t number) noexcept
{
    _currentLevelNumber = number;
}

std::size_t PlayerInfo::getLevelNumber() const noexcept
{
...
void PlayerInfo::setPlayer(Player* const player) noexcept
{
    _player = player;
}

Player* PlayerInfo::getPlayer() const noexcept
{
    return _player;
}
