...
void WorldContext::update(const sf::Time dt)
{
    switch (_currentLevelNumber)
    {
        case 1:
            ...
            if (_shadowBoss.mIsActive)
            {
                for (auto it = std::begin(_shadowBoss.mTentaclesStatic);
                    it != std::end(_shadowBoss.mTentaclesStatic);)
                {
                ...
                for (auto it = std::begin(_shadowBoss.mTentacles); it != std::end(_shadowBoss.mTentacles);)
                {
                    it->update(static_cast<float>(dt.asMilliseconds()));
                    if (it->mLife)
                    {
...
bool WorldContext::isGolemDarkActive() const noexcept
{
